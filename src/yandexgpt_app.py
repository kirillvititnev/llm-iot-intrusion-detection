...
def save_results(metrics, file_path):
    ...
    with open(file_path, 'w') as f:
        for model_name, metric in metrics.items():
            ...
            f.write(f"Classification Report: {metric['report']}\n")

# Сохранение лучшей модели
def save_best_model(models, metrics, file_path):
    """
    Сохраняет лучшую модель в файл.
    """
    best_model_name = max(metrics, key=lambda x: metrics[x]['f1'])
    dump(models[best_model_name], file_path)
...
if __name__ == '__main__':
    ...
    if csv_files:
        ...
        print("CSV-файлов не найдено в папке data/raw/")
        exit()
    ...
    # Создание папок для сохранения результатов и моделей
    ...
    # Сохранение результатов
    save_results(metrics, 'results/yandexgpt_metrics.txt')

    # Сохранение лучшей модели
    best_model_name = save_best_model(models, metrics, 'models/yandexgpt_best_model.pkl')

    # Вывод лучшего результата
    print(f"Лучшая модель: {best_model_name} с F1-score {metrics[best_model_name]['f1']}")
